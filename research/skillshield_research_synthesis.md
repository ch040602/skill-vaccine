# SkillShield Research Synthesis
...
- 향후 `skillshield manifest suggest`가 inferred capability를 manifest 초안으로 만든다.
...
- 후속 TODO는 `skillshield graph scan <skill_dir>`로 output risk tag와 input sensitivity tag를 연결한다.
